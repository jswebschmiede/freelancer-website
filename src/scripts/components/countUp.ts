import type { CountUpOptions } from '../../types';
...
class CountUp {
	...
	constructor(element: HTMLElement, options?: Partial<CountUpOptions>) {
		...
		this.finalValue = parseFloat(this.element.textContent ? this.element.textContent : '0');
	...
	initCountUp = () => {
		...
		const observer = new IntersectionObserver(this.countupObserve.bind(this), {
			threshold: [0, 0.1]
		});
	...
	countUpShow = () => {
		...
		if (parentCountup === null) {
			console.error('The parent node with class "countup" is null or not found.');
			return;
		}
	...
	initCountUpSr = () => {
		...
		if (!this.element || !this.element.parentNode) {
			console.error('The element or its parent node is null or undefined.');
			return;
		}
	...
	getCountupStart = () => {
		return this.deltaValue > 0 ? this.finalValue - this.deltaValue : this.initialValue;
	};
