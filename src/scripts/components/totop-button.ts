class totopButton {
  ...
  constructor(element: HTMLElement) {
    ...
    this.progressPath = this.element.querySelector('.svg-content path') as SVGPathElement;
  ...
  updateProgress = () => {
    ...
    const offset =
      this.totalLength - (scrollTop * this.totalLength) / (documentHeight - windowHeight);
    this.progressPath.style.strokeDashoffset = offset.toString();
  };

  updateProgressWrap = () => {
    if (window.scrollY > 50) {
      this.element.classList.remove('opacity-0', 'translate-y-5', 'invisible');
      this.element.classList.add('opacity-100', 'translate-y-0', 'visible');
    } else {
      this.element.classList.remove('opacity-100', 'translate-y-0', 'visible');
      this.element.classList.add('opacity-0', 'translate-y-5', 'invisible');
    }
  };
